...
export class DependencyValidator {
  ...
  static async validateProjectDependencies(
    ...
    try {
      ...
      result.availableRegistries =
        await DependencyValidator.getAvailableRegistries();

      // Check each dependency
      for (const dep of dependencies) {
        const isAvailable =
          await DependencyValidator.checkDependencyAvailability(dep);

        if (!isAvailable.available) {
          if (dep.required) {
            result.errors.push(
              `Required dependency ${dep.name}@${dep.version} is not available`
            );
            result.missingDependencies.push(dep.name);
            result.isValid = false;
          } else {
            result.warnings.push(
              `Optional dependency ${dep.name}@${dep.version} is not available`
            );
          }
      ...
      const hasWorkspaceDeps = dependencies.some((dep) =>
        dep.name.startsWith('@js-template-engine/')
      );
      if (hasWorkspaceDeps && result.availableRegistries.length === 0) {
        result.warnings.push(
          'No NPM registry configured. Workspace dependencies may not resolve.'
        );
      }
    } catch (error) {
      result.errors.push(
        `Dependency validation failed: ${error instanceof Error ? error.message : error}`
      );
      result.isValid = false;
    }
   ...
   */
  private static async checkDependencyAvailability(
    dep: DependencyInfo
  ): Promise<{
    available: boolean;
    registry?: string;
  ...
  static generateDependencyFixSuggestions(
    ...
    if (result.missingDependencies.length > 0) {
      ...
      if (workspaceDeps.length > 0) {
        ...
        suggestions.push(
          '   • Publish @js-template-engine/* packages to NPM first'
        );
      ...
      const externalDeps = result.missingDependencies.filter(
        (dep) => !dep.startsWith('@js-template-engine/')
      );
   ...
   */
  static async validateUIKitProject(
    projectDir: string
  ): Promise<DependencyValidationResult> {
    const packageJsonPath = path.join(projectDir, 'package.json');
    ...
    return await DependencyValidator.validateProjectDependencies(
      projectDir,
      dependencies
    );
  }
