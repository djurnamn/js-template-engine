 ...
 */
export interface LegacyProcessingOptions {
 ...
 */
export class LegacyProcessingPipeline {
  private registry: ExtensionRegistry;
  private analyzer: TemplateAnalyzer;
  private errorCollector: ErrorCollector;
  private performanceTracker: PerformanceTracker;
  private originalTemplate: TemplateNode[] = []; // Store original template for fallback rendering
  ...
  async process(
    ...
    options: LegacyProcessingOptions
  ): Promise<LegacyProcessingResult> {
    this.performanceTracker.start();
    this.errorCollector.clear();
    this.analyzer.clearErrors();
    
    // Store original template for fallback rendering
   ...
   */
  private getActiveExtensions(options: LegacyProcessingOptions): ActiveExtensions {
  ...
  private async renderOutput(
    ...
    if (!extensions.framework) {
      ...
      // Fallback to basic HTML rendering for backwards compatibility
      return this.renderBasicHTML(processedConcepts);
    }
  ...
  /**
   * Basic HTML renderer for fallback when no framework extension is available.
   * Provides backwards compatibility for basic template rendering.
   */
  private renderBasicHTML(processedConcepts: ProcessedConcepts): string {
    return this.renderTemplateNodes(this.originalTemplate);
  }

  /**
   * Render template nodes to basic HTML.
   */
   ...
   */
  private renderTemplateNode(node: TemplateNode): string {
    switch (node.type) {
      case 'text':
        return node.content || '';
      ...
      case 'element':
        ...
        const children = node.children ? this.renderTemplateNodes(node.children) : '';
      ...
      case 'if':
        // For basic rendering, always render the 'then' branch if available
        if (node.then) {
          return this.renderTemplateNodes(node.then);
        }
        return '';
      
      case 'for':
        // For basic rendering, render the template once
        if (node.children) {
          return this.renderTemplateNodes(node.children);
        }
        return '';
      
      case 'slot':
        // For basic rendering, use fallback if available
        if (node.fallback) {
          return this.renderTemplateNodes(node.fallback);
        }
        return '';
      
      case 'fragment':
        // Render fragment children directly
        if (node.children) {
          return this.renderTemplateNodes(node.children);
        }
        return '';
  ...
  /**
   * Render attributes to HTML attribute string.
   */
  private renderAttributes(attributes: Record<string, any>): string {
    ...
    for (const [key, value] of Object.entries(attributes)) {
      ...
      if (value === true) {
        ...
        pairs.push(`${key}="${String(value).replace(/"/g, '&quot;')}"`);
      }
    }
