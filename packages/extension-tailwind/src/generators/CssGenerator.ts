...
export class CssGenerator {
   ...
   */
  generateCssFromUtilities(
    utilities: ParsedUtility[],
    options: CssGenerationOptions = { format: 'css' }
  ): StyleOutput {
    ...
    try {
      ...
      console.warn(
        'Real Tailwind CSS generation failed, falling back to basic mappings:',
        error
      );
  ...
  generateScssWithApply(utilities: ParsedUtility[]): StyleOutput {
    ...
    if (groupedUtilities.base.length > 0) {
      scssLines.push(
        '@apply ' + groupedUtilities.base.map((u) => u.original).join(' ') + ';'
      );
    }

    // Pseudo-class variants
    for (const [variant, variantUtilities] of Object.entries(
      groupedUtilities.variants
    )) {
      if (variantUtilities.length > 0) {
        scssLines.push(`&:${variant} {`);
        scssLines.push(
          '  @apply ' + variantUtilities.map((u) => u.original).join(' ') + ';'
        );
        scssLines.push('}');
      }
    }
    ...
    // Responsive styles
    for (const [breakpoint, responsiveUtilities] of Object.entries(
      groupedUtilities.responsive
    )) {
      if (responsiveUtilities.length > 0) {
        const minWidth =
          this.breakpoints[breakpoint] || this.breakpoints.md || '768px';
        scssLines.push(`@media (min-width: ${minWidth}) {`);
        scssLines.push(
          '  @apply ' +
            responsiveUtilities.map((u) => u.original).join(' ') +
            ';'
        );
        scssLines.push('}');
      }
   ...
   * @returns Generated CSS output
   */
  private generateTailwindCssSync(
    utilities: ParsedUtility[],
    ...
    try {
      ...
      const result = this.tailwindProcessor.generateCssFromUtilities(
        allClasses,
        `.${className}`
      );
   ...
   * @returns Generated CSS output
   */
  private async generateTailwindCss(
    utilities: ParsedUtility[],
    options: CssGenerationOptions
  ): Promise<StyleOutput> {
    const groupedUtilities = this.groupUtilitiesByScope(utilities);
    ...
    try {
      ...
      for (const [breakpoint, responsiveUtilities] of Object.entries(
        ...
        if (responsiveUtilities.length > 0) {
          ...
          const minWidth =
            (options.breakpoints || this.breakpoints)[breakpoint] || '768px';

          // Handle mixed responsive + variant utilities
          const pureResponsive = responsiveUtilities.filter(
            (u) => u.variants.length === 0
          );
          const mixedUtilities = responsiveUtilities.filter(
            (u) => u.variants.length > 0
          );

          if (pureResponsive.length > 0) {
            const result =
              await this.tailwindProcessor.generateCssFromUtilities(
    ...
    switch (prefix) {
      ...
      case 'px':
        return spacingMap[value]
          ? `padding-left: ${spacingMap[value]}; padding-right: ${spacingMap[value]};`
          : null;
      case 'py':
        return spacingMap[value]
          ? `padding-top: ${spacingMap[value]}; padding-bottom: ${spacingMap[value]};`
          : null;
      case 'm':
      ...
      case 'mx':
        ...
        return spacingMap[value]
          ? `margin-left: ${spacingMap[value]}; margin-right: ${spacingMap[value]};`
          : null;
      case 'my':
        return spacingMap[value]
          ? `margin-top: ${spacingMap[value]}; margin-bottom: ${spacingMap[value]};`
          : null;
      case 'w':
