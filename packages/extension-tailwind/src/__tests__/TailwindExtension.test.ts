...
describe('TailwindExtension', () => {
  ...
  describe('coordinateWithFramework', () => {
    const mockFrameworkExtension = {
    ...
    it('should handle unknown classes based on configuration', () => {
      ...
      const result = extension.coordinateWithFramework(mockFrameworkExtension, concepts);
      expect(result.styling.staticClasses).toContain('bg-blue-500');
      expect(result.styling.staticClasses).toContain('custom-unknown-class');
    });

    it('should throw error for unknown classes when configured', () => {
      extension.options.unknownClassHandling = 'error';

      const concepts: ComponentConcept = {
        styling: {
          staticClasses: ['unknown-class'],
          dynamicClasses: [],
          inlineStyles: {}
        },
        events: [],
      ...
      expect(() => {
        extension.coordinateWithFramework(mockFrameworkExtension, concepts);
      }).toThrow('Unknown Tailwind class: unknown-class');
    });

    it('should ignore unknown classes when configured', () => {
      ...
      const result = extension.coordinateWithFramework(mockFrameworkExtension, concepts);
      expect(result.styling.staticClasses).toContain('bg-blue-500');
      expect(result.styling.staticClasses).not.toContain('unknown-class');
    });
  });
