/**
 * The Tailwind CSS v4 default theme values the conversion table resolves
 * against. Utility conversion is pinned to this theme; there is no theme
 * customization surface.
 */

/** Base unit of the numeric spacing scale (`p-4` → 4 × 0.25rem). */
export const SPACING_UNIT_REM = 0.25;

/** Responsive breakpoint variants, by prefix. */
...
/** The container width scale (`max-w-md`, `w-sm`, ...). */
export const CONTAINER_SCALE: Record<string, string> = {
  '3xs': '16rem',
  '2xs': '18rem',
  xs: '20rem',
  sm: '24rem',
...
/** Font size scale: each size carries its paired line height. */
...
/** Border radius scale; the bare `rounded` value is the v4 `--radius`. */
...
export const BARE_RADIUS = '0.25rem';

/**
 * Box shadow scale, resolved to plain `box-shadow` values (the composed
 * `--tw-shadow` machinery reduced to its defaults). The bare `shadow`
 * value is the v4 `--shadow`.
 */
...
export const DEFAULT_TRANSITION_TIMING_FUNCTION =
  'cubic-bezier(0.4, 0, 0.2, 1)';
export const DEFAULT_TRANSITION_DURATION = '150ms';

/** The full default color palette, plus the named non-palette colors. */
