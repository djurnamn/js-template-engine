...
export class TailwindProcessor {
   ...
   */
  async generateCssFromUtilities(utilities: string[], selector: string = '.component'): Promise<StyleOutput> {
  ...
  async isValidUtility(className: string): Promise<boolean> {
    try {
      ...
      const result = await this.postcssProcessor.process(testCSS, { from: undefined });
  ...
  getValidUtilityPrefixes(): Set<string> {
    ...
    return new Set([
      'bg', 'text', 'border', 'rounded', 'shadow', 'p', 'px', 'py', 'pt', 'pr', 'pb', 'pl',
      'm', 'mx', 'my', 'mt', 'mr', 'mb', 'ml', 'w', 'h', 'max-w', 'max-h', 'min-w', 'min-h',
      'flex', 'grid', 'block', 'hidden', 'inline', 'absolute', 'relative', 'fixed', 'sticky',
      'top', 'right', 'bottom', 'left', 'z', 'opacity', 'scale', 'rotate', 'translate',
      'skew', 'origin', 'overflow', 'cursor', 'pointer-events', 'select', 'resize',
      'font', 'tracking', 'leading', 'list', 'placeholder', 'caret', 'accent',
      'appearance', 'resize', 'scroll', 'snap', 'touch', 'will-change', 'outline', 'ring',
      'space', 'divide', 'decoration', 'underline', 'overline', 'line-through',
      'uppercase', 'lowercase', 'capitalize', 'normal-case', 'truncate', 'break',
      'whitespace', 'align', 'vertical', 'float', 'clear', 'object', 'filter',
  ...
  getColorTokens(): Record<string, string> {
    ...
    for (const [colorName, colorValue] of Object.entries(colors)) {
      if (typeof colorValue === 'string') {
        ...
        for (const [shade, hex] of Object.entries(colorValue as Record<string, string>)) {
  ...
  getValidVariants(): Set<string> {
    return new Set([
      ...
      'visited', 'checked', 'invalid', 'required', 'group-hover', 'group-focus',
      'focus-within', 'focus-visible', 'motion-safe', 'motion-reduce', 'dark',
      'print', 'portrait', 'landscape', 'first-letter', 'first-line',
      'selection', 'file', 'marker', 'before', 'after'
    ]);
  }
  ...
  async convertCssToTailwind(styles: Record<string, string>): Promise<{
    ...
    const reverseMap: Record<string, (value: string) => Promise<string | null>> = {
