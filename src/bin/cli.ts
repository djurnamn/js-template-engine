...
function loadExtensions(): Record<string, new (verbose?: boolean) => TemplateExtension> {
  const extensions: Record<string, new (verbose?: boolean) => TemplateExtension> = {};
  const fullPath = path.join(__dirname, '..', 'extensions');

  fs.readdirSync(fullPath).forEach((file) => {
    if (!['.js', '.ts'].includes(path.extname(file))) return;

    const extName = path.basename(file, path.extname(file));
    try {
      const ExtensionClass = require(path.join(fullPath, file)).default;
